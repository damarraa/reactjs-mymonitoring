...
import 'chartjs-plugin-datalabels'; // Import plugin datalabels
...
const TiangAtas = () => {
  ...
  useEffect(() => {
    api.get('/api/dashboard-tiangAtas')
      .then(response => {
        const data = response.data.map(item => ({
          label: item.kondisi_bagian_atas,
          value: item.count
        }));
        setSourceData(data);
  ...
  const options = {
    plugins: {
      title: {
        display: true,
        text: 'Kondisi Tiang Atas', // Judul chart
      },
      legend: {
